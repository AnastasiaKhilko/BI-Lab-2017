DROP TABLE Book_Author_M2M;
CREATE TABLE Book_Author_M2M
  (
...
COMMENT ON TABLE Book_Author_M2M
IS
...
';
  COMMENT ON column Book_Author_M2M.Book_author_id
IS
  'Table surrogate key';
  COMMENT ON column Book_Author_M2M.Product_id
IS
  'FK ON Dim_Products';
  COMMENT ON column Book_Author_M2M.Author_id
IS
  'FK ON Dim_Authors';
  COMMENT ON column Book_Author_M2M.IsAuthor
IS
  'Indicates whether the book was written BY one author OR many.';
  COMMENT ON column Book_Author_M2M.Insert_DT
IS
  'WHEN data was loaded';
  COMMENT ON column Book_Author_M2M.Update_DT
IS
  ' 
