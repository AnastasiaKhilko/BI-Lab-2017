create or replace PACKAGE BODY pkg_load_3nf_products AS
    ...
    BEGIN
        ...
        INSERT INTO wrk_products1 SELECT DISTINCT
            wrk.code, wrk.product, c2.subcategory_id, wrk.main_provider, wrk.country_provider,  trim('-' from wrk.discount) , wrk.price
...
CURSOR c_prod
    IS
  SELECT src.product_code,src.product_desc, src.subcategory_id, src.main_provider, src.country_provider, src.discount, src.price,
  ...
  WHERE (DECODE(src.product_code, tgt.product_code, 0,1)+ DECODE(src.product_desc, tgt.product_desc, 0,1) +
    DECODE(src.subcategory_id, tgt.subcategory_id, 0,1)+ DECODE(src.main_provider, tgt.main_provider, 0,1) + DECODE(src.country_provider, tgt.country_provider, 0,1) 
    + DECODE(src.discount, tgt.discount, 0,1) + DECODE(src.price, tgt.price, 0,1))>0
  union all
  SELECT tgt.product_code, tgt.product_desc, tgt.subcategory_id, tgt.main_provider, tgt.country_provider, tgt.discount, tgt.price,
...
OPEN c_prod;
  LOOP
   ...
   IF c_prod%found THEN
      INSERT INTO cls_products (product_code, product_desc, subcategory_id, main_provider, country_provider, discount, price,  start_dt, end_dt, isactive)
      values (rec_prod.product_code, rec_prod.product_desc, rec_prod.subcategory_id, rec_prod.main_provider, rec_prod.country_provider, rec_prod.discount, rec_prod.price,
...
BEGIN
  ...
  INSERT (product_id, product_code, product_desc, subcategory_id, main_provider, country_provider, discount, price, start_dt, end_dt, isactive)
  VALUES (bl_3nf.seq_products.nextval, src.product_code, src.product_desc, src.subcategory_id, src.main_provider, src.country_provider, src.discount, src.price, src.start_dt, src.end_dt, src.isactive);
    COMMIT;
