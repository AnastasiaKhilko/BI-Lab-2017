CREATE OR REPLACE PACKAGE pkg_load_dwh_customers
AS
  PROCEDURE load_cls2_cust;
  PROCEDURE load_dwh;
END pkg_load_dwh_customers;
/
CREATE OR REPLACE PACKAGE BODY pkg_load_dwh_customers
AS
PROCEDURE load_cls2_cust
IS
BEGIN
  ...
  SELECT customer_3nf_id,
    ...
    h.update_dt
  FROM bl_3nf.CE_customers h
  LEFT JOIN bl_3nf.CE_addresses ad
  ON h.ADDRESS_ID = ad.ADDRESS_ID
  LEFT JOIN bl_3nf.CE_cities c
  ON ad.CITY_ID = c.CITY_3NF_ID
  LEFT JOIN bl_3nf.CE_countries cc
  ON c.COUNTRY_ID = cc.COUNTRY_3NF_ID
  LEFT JOIN bl_3nf.CE_subregions r
  ON cc.SUBREGION_ID = r.SUBREGION_3nf_ID
  LEFT JOIN bl_3nf.CE_regions ss
  ON r.REGION_ID = ss.REGION_3NF_ID;
  COMMIT;
...
IS
BEGIN
  MERGE INTO bl_dwh.dim_customers d USING
  (SELECT customer_3nf_id,
    customer_id,
    gender,
    name,
  ...
  SELECT customer_3nf_id,
    ...
    REGION_NAME,
    update_dt
  FROM bl_dwh.dim_customers
  ) cls ON ( cls.customer_3nf_id = d.customer_3nf_id )
WHEN MATCHED THEN
  UPDATE
  SET d.customer_id    = cls.customer_id,
    d.gender           = cls.gender,
    d.name             = cls.name,
    ...
    d.update_dt        = cls.update_dt WHEN NOT MATCHED THEN
  INSERT VALUES
    (
      bl_dwh.seq_cust_dwh.nextval ,
      cls.customer_3nf_id,
      cls.customer_id,
      cls.gender,
      cls.name,
