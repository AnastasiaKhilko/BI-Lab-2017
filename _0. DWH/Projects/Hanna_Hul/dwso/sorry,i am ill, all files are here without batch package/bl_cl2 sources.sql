create or replace PACKAGE pkg_load_source_dwh AS
    PROCEDURE load_cls2_source;
    PROCEDURE load_dwh;
END pkg_load_source_dwh;
/
create or replace PACKAGE BODY pkg_load_source_dwh AS
...
PROCEDURE load_dwh
       IS
       begin
       MERGE INTO bl_dwh.dwh_sources d USING
( SELECT  source_3nf_id,
         source_id,
         Site,
         visits,
...
select *from bl_dwh.dwh_sources ;
select *from bl_3nf.CE_bankS ;
select bl_dwh.seq_source_dwh.nextval from dual ;
exec pkg_load_source_dwh.load_cls2_source;
exec pkg_load_source_dwh.load_dwh;