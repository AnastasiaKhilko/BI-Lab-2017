...
CREATE TABLE ext_customers
  ...
  (
    ...
    skip 1 nobadfile nodiscardfile nologfile fields terminated BY ',' 
    missing field VALUES are NULL ) LOCATION ('cust1.csv'
    -- ,'cust2.csv'
    )
  )
  REJECT LIMIT UNLIMITED;
   ...
   CREATE TABLE ext_hotels
    ...
    ORGANIZATION EXTERNAL
        (TYPE oracle_loader DEFAULT DIRECTORY ext_source
         ...
         nologfile fields terminated by ',' optionally enclosed by '"'
         missing field values are null
         )
         LOCATION ('hotel.csv')
    )
